...
%for l=1:L
%    B{l} = randn(size(R))/sqrt(L);
%    C{l} = ones(size(R))/sqrt(L);
%end
...
%     uker(m,1,1) = -1; uker(1,n,1)  =-1; uker(1,1,k) = -1;
...
ell = 0;
while(sum(iters) < maxiters && res_errors(end)>1e-4)
    ell = ell+1;
    ...
    for kay=1:3
        ...
        [u,~,~,iterc] = pcg(AtA, reshape(rhs,[numel(rhs) 1]), 1e-7, 100);
