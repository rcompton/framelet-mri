function [A, lambda, V] = laplacian(varargin)

% LAPLACIAN   Sparse Negative Laplacian in 1D, 2D, or 3D
%
%    A = LAPLACIAN(N) generates a sparse negative 3D Laplacian matrix with
...
%
%    [A,LAMBDA,V] = LAPLACIAN(N,B,M) produces a 2D or 1D negative
